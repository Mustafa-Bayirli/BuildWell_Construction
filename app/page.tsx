...
export default function HomePage() {
  return <main className="overflow-hidden bg-[#f4f5f7] text-[#252525]">
    <section className="relative min-h-[720px] bg-[#071a33] text-white">
      ...
      <div className="absolute bottom-0 left-0 right-0 grid border-t border-white/15 bg-[#071a33]/80 sm:grid-cols-3">{[{ icon: ShieldCheck, title: "Quality craftsmanship" }, { icon: Clock3, title: "End-to-end management" }, { icon: Building2, title: "Commercial · Industrial · Residential" }].map(({ icon: Icon, title }) => <div key={title} className="flex items-center gap-3 border-b border-white/10 px-6 py-5 text-sm font-medium sm:border-b-0 sm:border-r sm:px-10"><Icon className="h-5 w-5 text-[#c9a23a]" />{title}</div>)}</div>
    </section>
